...
pub fn abc_output_min(mut difference_min: HashMap<String , (Vec<String> , HashMap<String, String> , Vec<String> )> , sep2_2_size_parameters: String) {

    let x: HashMap<String , (Vec<String> , HashMap<String, String> , Vec<String> )> = difference_min;
    ...
    println!("        ↓        ↓        ↓        ↓");
    println!("##### abc_output_min.rs -> abc_output_min  #####");
    for (key, value ) in &x {
        for zero in &value.0 {
...
//  }
    ...
    let mut abc_output_min_time_list: Vec<String> = Vec::new();
    for every in &ac {
        let mut ev = every.clone();
        let v: Vec<_> = ev.match_indices(&sep2_2_size_parameters).collect();
        let a = v[0].0 + 1;
        let zz = ev.split_off(a);
        abc_output_min_time_list.insert(0, zz);

    }
    abc_output_min_time_list.sort();
    abc_output_min_time_list.dedup();


    println!("defect time list");
    for a in &abc_output_min_time_list{
        println!("\t\t{:?}", a );
    }

    let mut sometime_vec: Vec<String> = Vec::new();
    for a_two in abc_output_min_time_list {
        for eve_two in &ac {
        ...
        for stv in &sometime_vec {
            let mut sv = stv.clone();
            let v: Vec<_> = stv.match_indices("_tpd_").collect();
            if v.len() == 1 {
                let b = v[0].0;
...
//  let mut hmap: HashMap<String, HashMap<String, String>>  = HashMap::new();
    ...
    for i in &output_bao_list {
        ...
        for aj in time_list {
            ...
            for mut stv in &samebaosametime {
                let mut sv = stv.clone();
                for j in &["_hntpd", "_hn_tpd_smsc_traf_delay", "_tpd"] {
    ...
    let mut abc_output_min_count = 0;
    for (key, value) in &hmap {
        abc_output_min_count  += 1;
        println!("");
        println!("{} is key si {} ", key, abc_output_min_count );
        println!("|");
        println!("V");
        for (m , n ) in value {
            println!("{}", m);
