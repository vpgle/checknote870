...
pub fn abc_output_5gc2(mut difference_5gc2: HashMap<String , (Vec<String> , HashMap<String, String> , Vec<String> )> , sep2_2_size_parameters: String) {

    let x: HashMap<String , (Vec<String> , HashMap<String, String> , Vec<String> )> = difference_5gc2;
    ...
    println!("        ↓        ↓        ↓        ↓");
    println!("##### abc_output_5gc2.rs -> abc_output_5gc2  #####");
    for (key, value ) in &x {
        for zero in &value.0 {
...
//  }
    ...
    let mut abc_output_5gc2_time_list: Vec<String> = Vec::new();
    for every in &ac {
        let mut ev = every.clone();
        let v: Vec<_> = ev.match_indices(&sep2_2_size_parameters).collect();
        let a = v[0].0 + 1;
        let zz = ev.split_off(a);
        abc_output_5gc2_time_list.insert(0, zz);

    }
    abc_output_5gc2_time_list.sort();
    abc_output_5gc2_time_list.dedup();


    println!("defect time list");
    for a in &abc_output_5gc2_time_list{
        println!("\t\t{:?}", a );
    }

    let mut sometime_vec: Vec<String> = Vec::new();
    for a_two in abc_output_5gc2_time_list {
        for eve_two in &ac {
