...
//        文件名带时间                  时间戳  文件大小
pub fn average_value_min(riqi: &str, sep2_size_parameters: &str, sep2_1_size_parameters: &str) -> HashMap<String, u32> {
...
//    let mut hmap = HashMap::new();
    ...
    println!("average_value_min.rs -> riqi is {}", &riqi);
...
//  println!("average_value_min.rs -> y_ear is {:?}", y_ear);
//  println!("average_value_min.rs -> x is {:?}", x );
    println!("average_value_min.rs -> average_value_min -> v_sep1_filename is |{:?}|", v_sep1_filename);
//  let mon = local.format("%b").to_string();
    let mut sep2_size = sep2_size_parameters.to_string();
    println!("average_value_min.rs -> average_value_min -> sep2_size is |{}|", sep2_size );
    let mut sep2_1_size = sep2_1_size_parameters.to_string();
    println!("average_value_min.rs -> average_value_min -> sep2_1_size is |{}|", sep2_1_size );

    // 获得今天的所有   分钟粒度 文件
    //  hxwxn_min  来源 mecheck_note 模板 中 的 分钟 粒度
    let hxwxn_min: Vec<String> = hxwxn_zero_min::hxwxn_min();
    println!("average_value_min.rs > todaylist.rs > todayhourlist > hxwxn_min length is {}", &hxwxn_min.len());
    ...
    for j in &hxwxn_min {
        ...
        for line in fin.lines() {
            ...
            let s_j = String::new();
//          println!("average_value_min.rs -> &riqi_bak is {}", &riqi_bak);
//          println!("average_value_min.rs -> h is {}", h);
...
//                  println!("average_value_min.rs -> inner_hmap is {:?}", inner_hmap);
//                  println!("average_value_min.rs -> inner_hmap_bak is {:?}", inner_hmap_bak);

                    // hashmap_timestamp_size : Vec<>
                    // println!("average_value_min.rs -> hashmap_timestamp_size  before is {:?}", &hashmap_timestamp_size);
...
//                  println!(" ###  hashmap_timestamp_size is ->   {:?} length is {}", &hashmap_timestamp_size, &hashmap_timestamp_size.len());
                    ...
                    // println!("average_value_min.rs -> hashmap_timestamp_size after is {:?}", hashmap_timestamp_size);
...
//  println!("average_value_min.rs -> hashmap_filename_timestamp_size is {:?}", &hashmap_filename_timestamp_size.len());
    ...
    println!("在average_value_min.rs 中 生成 ");
    for (key , value ) in &hashmap_filename_timestamp_size {
//      println!("key is {:?} <<<< average_value_min.rs >>>> value is {:?}", key, value );
...
//      因定义 store_value 时是 可变的，所有可以不用 clone 到 aver_store_value ，便可直接使用 store_value
        ...
        // 最大值和最小值，以此往复
//      println!(" aver_store_value is {:?}", &aver_store_value );
