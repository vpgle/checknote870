...
//        文件名带时间                  时间戳  文件大小
pub fn average_value(riqi: &str, sep2_size_parameters: &str, sep2_1_size_parameters: &str) -> HashMap<String, u32> {
...
//    let mut hmap = HashMap::new();
    ...
    println!("average_value.rs -> riqi is {}", &riqi);
...
//  println!("average_value.rs -> y_ear is {:?}", y_ear);
//  println!("average_value.rs -> x is {:?}", x );
    println!("average_value.rs -> average_value -> v_sep1_filename is |{:?}|", v_sep1_filename);
    let mut sep2_size = sep2_size_parameters.to_string();
    println!("average_value.rs -> average_value -> sep2_size is |{}|", sep2_size );
    let mut sep2_1_size = sep2_1_size_parameters.to_string();
    println!("average_value.rs -> average_value -> sep2_1_size is |{}|", sep2_1_size );
    ...
    println!("average_value.rs > todaylist.rs > todayhourlist > hxwxn_zero length is {}", &hxwxn_zero.len());
    ...
    for j in &hxwxn_zero {
        ...
        for line in fin.lines() {
            ...
            let s_j = String::new();
//          println!("average_value.rs -> &riqi_bak is {}", &riqi_bak);
//          println!("average_value.rs -> h is {}", h);
//          println!("v_sep1_filename is {:?}", v_sep1_filename);
            ...
            if new_line.contains(&h) && new_line.contains("zip") {
                ...
                if  v_a.len() > 0{
                    ...
                    println!("average_value.rs 里没有找到 年分隔符 - v_sep1_filename!!!");
                }
...
//                  println!("new_filename is  {}", new_filename);
                    ...
                    let mut timestamp = timestamp.parse::<u128>().unwrap();
...
//
    ...
    //                          println!("\t\t|size| is |{}|", size );
                                ...
                                println!("new_line one is {:?}" , new_line);
    ...
    //                          println!("\t\t|size| is |{}|", size );
                                ...
                                println!("new_line  two is {:?}" , new_line);
                                println!("找不到获取文件大小的 sep2_1_size 分隔符!");
                            }
                        } else {
                            println!("new_line end is {:?}" , new_line);
...
//                  println!("average_value.rs -> inner_hmap is {:?}", inner_hmap);
//                  println!("average_value.rs -> inner_hmap_bak is {:?}", inner_hmap_bak);

                    // hashmap_timestamp_size : Vec<>
                    // println!("average_value.rs -> hashmap_timestamp_size  before is {:?}", &hashmap_timestamp_size);
...
//                  println!(" ###  hashmap_timestamp_size is ->   {:?} length is {}", &hashmap_timestamp_size, &hashmap_timestamp_size.len());
                    ...
                    // println!("average_value.rs -> hashmap_timestamp_size after is {:?}", hashmap_timestamp_size);
                    ...
                    // println!("\taverage_value.rs -> hashmap_filename_timestamp_size is {:?}", hashmap_filename_timestamp_size);
...
//  println!("average_value.rs -> hashmap_filename_timestamp_size is {:?}", &hashmap_filename_timestamp_size.len());
    ...
    println!("在average_value.rs 中 生成 ");
    for (key , value ) in &hashmap_filename_timestamp_size {
//      println!("key is {:?} <<<< average_value.rs >>>> value is {:?}", key, value );
